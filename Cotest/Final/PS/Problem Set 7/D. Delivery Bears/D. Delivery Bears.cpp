...
int main() {
    ...
    auto check = [&] (double mid) -> bool {
        FlowNetwork network = FlowNetwork(n + 1);
        for (auto& edge : edges) {
            long long bear_capacity = (long long) (edge.c / mid);
            if (bear_capacity > 0) {
                network.add_edge(edge.a, edge.b, bear_capacity);
            }
        }
        return network.dinic(1, n) >= x;
    };
    ...
    double total_weight = x * lo;

    cout << fixed << setprecision(10) << total_weight << '\n';
}